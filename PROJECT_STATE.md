...
- `ingest-staging` 내 저장 경로는 `차종/yyyy/mm/dd/yyddmm_차번호/` 규칙을 따른다.
- 저장 경로 예시는 `U100/2026/04/08/260407_v012/` 이다.
- `pcap` 파일명은 원본을 유지한다.
...
- 기존 task 문서 재검토
- Codex 첫 구현 요청문 재정비
- ingest-staging 저장 경로 규칙 반영

### 5.3 미착수 항목

- Maven 기반 실제 Java 프로젝트 skeleton 생성
- `app`, `config` 패키지의 실제 코드 작성
- `device`, `scan` 패키지의 실제 코드 작성
- `storage` 패키지의 실제 코드 작성
- `naming`, `logging` 패키지의 실제 코드 작성
- `udev` / `libudev` / JNA 실제 연동 구현
- MinIO 실제 업로드 구현
- 테스트 코드 작성
- 첫 번째 실제 구현 사이클 수행
...
다만 업로드 대상 object key를 `차종/yyyy/mm/dd/yyddmm_차번호/` 규칙에 맞게 안정적으로 계산하고, 운영 입력값과 파일명 규칙을 일관되게 반영할 수 있는지 구현 시 확인이 필요하다.

### 9.8 src-extracted 경로 규칙 변경 가능성

`ingest-staging` 대상 폴더명 및 경로 규칙은 현재 기준으로 `차종/yyyy/mm/dd/yyddmm_차번호/` 형태를 따른다.
다만 차량번호 표기 방식, 날짜 해석 기준, object key 조합 규칙의 세부 기준이 추가될 경우 구현 구조와 테스트 기준에 영향을 줄 수 있다.

### 9.9 udev 연동 계층 의존성
장치 감지는 `libudev`와 JNA 연동을 전제로 하므로, 운영 환경에서 해당 연동 방식의 호환성과 배포 구성을 확인해야 한다.

## 10. 현재 우선 순위
1. 변동사항 반영에 따른 문서 최신화
2. `PROJECT_STATE.md` 최신화
3. `README.md`, `PRD.md`, `ARCHITECTURE.md` 정합성 점검 및 보정
4. 구현 task 문서 1~4 재정리
5. `AGENT_EXECUTION_REQUEST_01.md` 갱신
6. Codex 첫 구현 사이클 시작
7. 실제 Java daemon skeleton 생성
...
- `PROJECT_STATE.md` 최신화 완료
- `README.md` 최신화
- `docs/project/PRD.md`와 `docs/project/ARCHITECTURE.md` 최신화
- 구현 task 문서 1~4 최신화
- `docs/project/AGENT_EXECUTION_REQUEST_01.md` 갱신
- Codex 첫 구현 요청 실행
- Maven 기반 Java daemon 초기 코드 구조 생성
- 첫 구현 결과에 대한 테스트 및 리뷰 루프 시작
