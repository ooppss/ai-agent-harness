...
본 daemon의 목적은 USB 저장소 연결을 감지하고, 해당 장치에서 파일 탐색 가능한 경로가 확인되면 장치 내부 지정 경로에서 `*.pcap` 파일을 탐색하여 MinIO `ingest-staging` bucket의 지정 경로로 업로드하는 것이다.

본 제품의 현재 책임 범위는 `ingest-staging` 업로드까지이며, `src-extracted` 관련 처리는 현재 범위에 포함하지 않는다.

### 2.2 주요 목표
- USB 저장소 연결을 감지한다.
- 장치의 파일 탐색 가능한 경로를 확인한다.
...
### 5.1 전체 처리 흐름
1. USB 저장소가 서버에 연결된다.
2. Ubuntu가 해당 장치에 대한 `udev` 이벤트를 발생시킨다.
3. daemon이 `udev` 이벤트를 통해 USB 저장소 연결을 감지한다.
4. daemon이 해당 장치에서 파일 탐색 가능한 경로 확인을 시도한다.
5. 파일 탐색 가능한 경로가 즉시 확인되지 않으면 짧은 재시도를 수행한다.
6. daemon이 확인된 경로 하위의 지정 경로를 탐색한다.
...
## 8. 기능 요구사항
- FR-01. daemon은 USB 저장소 연결을 감지해야 한다.
- FR-02. daemon은 장치 이벤트 직후 파일 탐색 가능한 경로를 확인해야 하며, 즉시 확인되지 않으면 짧은 재시도를 수행해야 한다.
...
- NFR-04. USB 저장소 감지는 Linux `udev` 이벤트와 `libudev` + JNA 연동을 기준으로 한다.
- NFR-05. 오류 발생 시 원인 추적이 가능한 수준의 로그를 남겨야 한다.

## 10. 오류 처리 요구사항
- ER-01 파일 탐색 가능 경로 미확인 : 파일 탐색 가능한 경로를 찾지 못하면 처리 중단 후 오류를 로그에 기록해야 한다.
...
## 11. 수용 기준
- AC-01. USB 저장소가 연결되면 daemon이 이를 감지할 수 있어야 한다.
- AC-02. daemon이 파일 탐색 가능한 경로를 확인하고 지정 경로에서 `*.pcap` 파일을 탐색할 수 있어야 한다.
...
- OI-01 파일 탐색 가능한 경로 확인 방식 : 실제 운영 서버에서 USB 저장소가 어떤 방식으로 파일 탐색 가능한 상태가 되는지, 그리고 `/proc/self/mountinfo` 우선 + `/proc/self/mounts` fallback 방식이 안정적으로 동작하는지 운영 환경 확인이 필요하다.
- OI-02 USB 저장소 식별 규칙 : 실제 운영 서버에서 `SUBSYSTEM=block`, `ID_BUS=usb`, `DEVTYPE=partition/disk` 기준이 안정적으로 적용되는지 운영 환경 확인이 필요하다.
