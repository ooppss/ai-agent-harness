...
1. 서버에 USB 저장소가 연결되면 이를 감지한다.
2. 해당 장치에서 파일 탐색 가능한 경로를 확인한다.
3. 파일 탐색 가능한 경로가 확인되면 장치 내부의 지정 경로에서 `*.pcap` 파일을 탐색한다.
...
- Java daemon 개발을 수행할 수 있는 제품 문서와 구현 task 문서를 최신 상태로 유지한다.
- 확정된 object key 규칙과 중복 처리 정책을 문서와 코드에 일관되게 반영한다.
- naming / storage / logging 품질을 높이기 위한 테스트 자산을 보강한다.
- device → scan → storage 흐름의 통합 검증 수준을 높인다.

현재 프로젝트는 운영 문서, 제품 문서, 구현 task 문서, 실행 요청 문서, 1차 구현 코드, 핵심 단위 테스트가 반영된 상태이며, 다음 구현 사이클을 준비 중이다.
...
- USB 저장소 감지는 Linux `udev` 이벤트를 기준으로 한다.
- USB 저장소 처리 시작 신호는 기본적으로 `add` 이벤트를 사용한다.
- 처리 후보 장치는 `SUBSYSTEM=block` 및 `ID_BUS=usb` 조건을 만족하는 장치로 제한한다.
- 가능하면 `DEVTYPE=partition`을 우선 사용하고, 필요 시 `disk`도 허용한다.
...
- USB 저장소 연결을 감지 대상으로 한다.
- 감지 대상은 USB 메모리, 외장 SSD, 외장 HDD, USB 카드리더기를 통해 인식된 저장소를 포함한다.
- 장치 감지는 이벤트 기반으로 처리한다.
- 장치 감지는 `udev` 이벤트를 기준으로 한다.
- 처리 시작 신호는 기본적으로 `add` 이벤트를 사용한다.
- 처리 후보 장치는 `SUBSYSTEM=block` 및 `ID_BUS=usb` 조건을 만족하는 장치로 제한한다.
- 가능하면 `DEVTYPE=partition`을 우선 사용하고, 필요 시 `disk`도 허용한다.
- Java는 `libudev`와 JNA 연동 방식으로 장치 이벤트를 처리한다.
- 실제 처리 시작은 해당 장치에서 파일 탐색 가능한 경로가 확인된 시점으로 한다.
- 파일 탐색 가능한 경로는 이벤트 직후 확인하며, 즉시 확인되지 않으면 짧은 재시도를 수행한다.
- 파일 탐색 가능한 경로 확인은 `/proc/self/mountinfo`를 우선 사용하고, 필요 시 `/proc/self/mounts`를 fallback으로 사용한다.
- 초기 구현은 단일 장치 처리 방식으로 제한한다.
- 확인된 경로 하위의 지정 경로에서 `*.pcap` 파일을 탐색한다.
...
- 중복 object key 정책의 문서/코드 반영 마무리
- naming / storage / logging 테스트 보강 방향 정리
- 다음 구현 사이클 범위 정리

### 5.3 미착수 항목

- object key 세부 예외 규칙 처리
- 중복 object skip 결과 기록 방식의 상세 기준 반영
...
현재 daemon은 USB 저장소 연결 감지, 파일 탐색 가능한 경로 확인, `pcap` 파일의 수집 대상 탐색, `ingest-staging` bucket 업로드까지만 담당한다.
...
- `SUBSYSTEM=block`, `ID_BUS=usb`, `DEVTYPE=partition/disk` 필터 기준의 실제 운영 환경 검증 결과
- 파일 탐색 가능한 경로 확인 방식의 실제 운영 환경 규칙
- 로그 파일 정책과 운영 로그 보존 방식의 상세 기준
- 운영 스크립트 배치 방식의 상세 기준
- 중복 object 처리 시 skip 결과 기록 방식의 상세 기준
...
### 9.5 파일 탐색 가능 경로 확인 방식 차이 가능성

실제 운영 서버에서 USB 저장소가 파일 탐색 가능한 상태가 되는 방식과 경로는 환경마다 다를 수 있다.
이 차이는 구현 방식과 테스트 방식에 직접 영향을 줄 수 있다.

### 9.6 장치 감지 방식의 구현 복잡도

장치 감지는 이벤트 기반의 `udev` 연동 방식으로 확정되었다.
다만 USB 저장소 식별, `SUBSYSTEM=block`/`ID_BUS=usb`/`DEVTYPE` 필터 적용, `libudev`와 JNA 연동, 파일 탐색 가능한 경로 재시도 처리, 운영 환경별 준비 타이밍 차이로 인해 구현 복잡도가 존재한다.
...
또한 실제 Ubuntu 20.04 환경에서 `ID_BUS=usb`, `DEVTYPE`, `SUBSYSTEM=block` 기준이 USB 저장소 식별에 안정적으로 적용되는지 검증이 필요하다.

## 10. 현재 우선 순위
1. 중복 object skip 정책의 코드 반영 완료
2. naming / storage / logging 테스트 보강
3. object key 세부 예외 규칙 정리
4. MinIO 통합 테스트 보강
...
- 중복 object skip 정책의 코드 반영 완료
- object key 세부 예외 규칙 정리
- naming / storage / logging 테스트 보강
- MinIO 통합 테스트 보강
- device → scan → storage 통합 검증 강화
- `udev` 실제 연동 정교화
- 다음 구현 사이클 실행
