...
`차종/yyyy/mm/dd/yyddmm_차번호/`

예:
- `U100/2026/04/08/260407_v012/`

각 `pcap` 파일은 위 경로 아래에 원본 파일명 그대로 저장한다.

### 7.2 object key 구성 원칙
daemon은 입력 데이터에서 확인 가능한 차량 유형, 날짜, 차량번호 정보를 이용해 업로드 대상 object key를 계산해야 한다.
...
### 7.3 중복 처리
중복 object 처리 정책의 최종 기준은 현재 운영 기준 확정이 필요하다.
...
- FR-05. daemon은 업로드 대상 object key를 `차종/yyyy/mm/dd/yyddmm_차번호/` 규칙과 원본 `pcap` 파일명을 조합하여 계산해야 한다.
...
- AC-04. 업로드 경로가 `차종/yyyy/mm/dd/yyddmm_차번호/` 규칙과 일치해야 한다.
- AC-05. 업로드된 object 이름이 원본 `pcap` 파일명과 동일해야 한다.
- AC-06. `src-extracted` 관련 처리 없이도 현재 요구사항을 만족하는 것으로 판정해야 한다.
...
- OI-03 ingest-staging object key 세부 규칙 : 차량번호 표기 방식, 날짜 해석 기준, object key 세부 조합 규칙의 운영 기준 확정이 필요하다.
- OI-04 중복 object 처리 정책 : 동일 object key 존재 시 overwrite 여부 또는 대체 처리 방식의 운영 기준 확정이 필요하다.
