...
- **Device Detection Module**
  - Linux `udev` 이벤트를 기준으로 USB 저장소 연결 감지
  - `libudev`를 JNA로 연동하여 이벤트 수신
  - 파일 탐색 가능한 경로 확인 흐름 시작

- **Device Scan Module**
  - 파일 탐색 가능한 경로 하위 지정 경로 탐색
  - `*.pcap` 파일 식별
...
### 4.1 USB 저장소 → ingest-staging
1. Device Detection Module이 `udev` 이벤트를 통해 USB 저장소 연결을 감지한다.
2. 파일 탐색 가능한 경로 확인을 시도한다.
3. 파일 탐색 가능한 경로가 즉시 확인되지 않으면 짧은 재시도를 수행한다.
4. Device Scan Module이 확인된 경로 하위의 지정 경로를 탐색한다.
...
### 5.6 USB 저장소 감지 및 처리 범위 규칙
- USB 저장소 연결 감지는 이벤트 기반으로 처리한다.
- Linux `udev` 이벤트를 기준으로 동작한다.
- 실제 처리 시작은 파일 탐색 가능한 경로가 확인된 시점으로 한다.
- 파일 탐색 가능한 경로가 즉시 확인되지 않으면 짧은 재시도를 수행한다.
...
### 6.1 운영체제
- Ubuntu 20.04
- `udev` 이벤트 기반 USB 저장소 감지가 가능해야 한다.
- USB 저장소에 대해 파일 탐색 가능한 경로 확인이 가능해야 한다.
...
## 9. 구현 제약
- USB 저장소 감지는 `udev` 이벤트 기반으로 구현한다.
...
## 10. 테스트 관점의 핵심 포인트
- USB 저장소 연결 후 파일 탐색 가능한 경로 확인 가능 여부
- 지정 경로의 `pcap` 탐색 여부
...
## 11. 향후 확장 가능 지점
- USB 저장소 감지 및 파일 탐색 가능한 경로 확인 구현을 더 안정적인 운영 방식으로 고도화
- 다중 장치 동시 처리
- 처리 이력 영속 저장
- 관리자용 상태 조회 CLI 추가
- 후속 앱 연계 인터페이스 추가