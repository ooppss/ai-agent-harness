...
- OI-01 파일 탐색 가능한 경로 확인 방식 : 실제 운영 서버에서 USB 저장소가 어떤 방식으로 파일 탐색 가능한 상태가 되는지 운영 환경 확인이 필요하다.
- OI-02 USB 저장소 식별 규칙 : 실제 운영 서버에서 대상 USB 저장소를 어떤 속성으로 식별할지 운영 환경 확인이 필요하다.
