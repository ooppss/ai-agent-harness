...
- 업로드 대상 경로는 `차량유형/yyyy/mm/dd/yymmdd_v차량번호/` 규칙을 반영할 수 있는 구조로 준비한다.
- 기본 object key 규칙은 코드에 반영하되, 세부 예외 규칙은 과도하게 하드코딩하지 않는다.
- 파일명은 원본 `pcap` 파일명을 유지한다.
- 동일 object key가 이미 존재하면 해당 파일은 overwrite 하지 않고 skip 처리해야 한다.
...
- 결과 구조는 success / failure 외에 skip 상태도 구분할 수 있는 것을 권장한다.
